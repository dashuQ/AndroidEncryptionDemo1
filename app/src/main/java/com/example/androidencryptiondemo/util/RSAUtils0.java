...
public class RSAUtils0 {
	...
	public static byte[] encrypt(String publicKey, byte[] plainTextData)
		...
		try {
			// 使用默认RSA
//			cipher = Cipher.getInstance(RSA_ANDROID);//"RSA"
			cipher = Cipher.getInstance(RSA_JAVA);//"RSA"
//			cipher = Cipher.getInstance(RSA);//"RSA"
	...
	public static byte[] decrypt(String privateKey, byte[] cipherData)
		...
		try {
			...
			cipher = Cipher.getInstance(RSA_ANDROID);//"RSA"
//			cipher = Cipher.getInstance(RSA_ANDROID);//"RSA"
			cipher.init(Cipher.DECRYPT_MODE, loadPrivateKeyByStr(privateKey));
